...
### Fixed
- **Issue with dictionary keys being unquoted** - Dictionary keys were not quoted, leading to errors when loading said dictionaries in the variables. Dictionary keys are now properly quoted.

## [0.5.2] - 2025-11-10

### Added
- **Support for `nil` values in variables** - Elixir `nil` values are now properly converted to Typst's `none` value, allowing optional fields to be passed as `nil` in variable maps
